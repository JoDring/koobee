            ...
            <div class="detail-information-list" v-if="sampleList && sampleList.length > 0">
                <div class="list-item" v-for="item in sampleList" @click="goToDetail(item)">
                    ...
                    <div class="list-item-txt">
                        ...
                        <div class="item-time">
                            <span v-if="item.appName">《{{item.appName}}》</span>
                            <span>{{item.timeStr}}</span>
                        </div>
                    </div>
                </div>
                <router-link :to="{name:'InformationListV2', params: {resetScroller: true}}" class="list-bottom-btn">查看更多</router-link>
            </div>
        </main>
...
<script>
    ...
    import {fetchInformationListV2, fetchInformationDetail} from '../services/appStore'
                ...
                return fetchInformationDetail({id: appId}).then(res => {
                    ...
                    if (res.code === '0') {
                        ...
                        if (!this.informationList || this.informationList.length === 0) {
                            fetchInformationListV2({pageIndex: 1, pageSize: 100}).then(res => {
                                if (res.code === '0') {
