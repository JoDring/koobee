<template>
  <view-box ref="viewBox">
    <template v-if="appList.length > 0">
      <x-header v-if="$route.query.isSub" slot="header" :left-options="{backText: ''}">{{appList.length && appList[0].name}}</x-header>
      <section class="section-brief">
        <div class="blur-img-c">
          <x-img container="#vux_view_box_body" class="blur-img" default-src="static/images/palceholder-logo.png" :src="appList[0].iconUrl" width="180" height="180"></x-img>
          <div class="mask-color"></div>
          <img class="mask-img" src="static/images/bg-mask.png" width="720" height="122">
        </div>
        <div class="logo-c">
          <x-img container="#vux_view_box_body" class="logo-big" default-src="static/images/palceholder-logo.png" :src="appList[0].iconUrl" width="180" height="180"></x-img>
          <div class="inverted-logo-c">
            <x-img container="#vux_view_box_body" class="logo-big inverted" default-src="static/images/palceholder-logo.png" :src="appList[0].iconUrl" width="180" height="180"></x-img>
          </div>
        ...
        <div class="app-down-btn">
          <btn-download :pause="appList[0].btnState.pause" :start-load="appList[0].btnState.startLoad"
                        :percentage="appList[0].btnState.percentage" :url="appList[0].downloadUrl"></btn-download>
        </div>

      </section>
      <section class="app-images">
        <x-img v-for="item in imgList" class="app-images-item" default-src="static/images/placeholder-appbg.png" :src="item" :key="item" container=".app-images"></x-img>
      </section>
      ...
      <a href="#" class="arrow-down-c" @click.prevent="showMore = !showMore">
        <img src="../assets/arrow-down.png" class="arrow-down" width="60" height="60" :class="{rotated: showMore}">
      </a>
          ...
          <flexbox :gutter="0">
            <flexbox-item :span="3" class="ad-list-item" v-for="item in sameCatApps" :key="item.id">
              <router-link :to="{name:'AppDetail', params: {appId: item.id}, query: {isSub: true}}">
                <div class="logo-sm-c">
                  <x-img class="logo-sm" default-src="static/images/palceholder-logo.png" :src="item.iconUrl" container="#vux_view_box_body" width="130"
                         height="130"></x-img>
                </div>
              </router-link>
              <h3>{{item.name}}</h3>
...
<script>
  ...
  const url = /szprize\.cn/i.test(location)? 'http://appstore.szprize.cn/appstore/appinfo/details?appId=' : 'http://192.168.1.148:8080/appstore/appinfo/details?appId=';
  export default {
    name: 'AppDetail',
    props: {
      appId: {
