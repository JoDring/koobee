<template>
    ...
    <div class="information-app-ad" v-if="app">
        <div class="app-info-brief" @click="goToDetail">
            <img :src="app.iconUrl" class="app-info-icon">
            <div class="brief-txt">
                <span>{{app.name}}</span>
                <span class="txt-gray">{{app.apkSize | formatSize(2)}}</span>
            </div>
        </div>
...
<script>
    ...
    export default {
        ...
        methods: {
            goToDetail() {
                if (window.jsObj) {
                    JsCallApp.handleAppAction(JSON.stringify(this.app), 'detail');
                    window.MtaH5 && window.MtaH5.clickStat('game_to_appdetail_from_info', {
                        'id': this.app.id,
                        'name': this.app.name,
